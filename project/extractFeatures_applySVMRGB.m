function [featureExtracted,error] = extractFeatures_applySVMRGB(X1train,X1val,X2train,X2val,X3train,X3val,X4train,X4val)
    ...
    [error,I] = sort(auxerr);%,'descend');%error ordinati
    featureExtracted = [];
    
    for i = 1:8
        featureExtracted = cat(1,featureExtracted,auxft(I(i,1)));%Features ordinate
    end
